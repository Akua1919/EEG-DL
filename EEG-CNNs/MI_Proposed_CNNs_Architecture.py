...
with tf.name_scope("Input"):
    ...
    with tf.name_scope("Reshape_Data"):
        x_Reshape = tf.reshape(tensor=x, shape=[-1, 50, 20, 1])
...
with tf.name_scope('Pooling_1'):
    with tf.name_scope('h_pool3'):
        h_pool3 = tf.nn.max_pool(h_conv3_drop, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')     #25*10*64
...
with tf.name_scope('Pooling_2'):
    with tf.name_scope('h_pool6'):
        h_pool6 = tf.nn.max_pool(h_conv6_drop, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')     #12*4*128
        print(h_pool6)
...
with tf.name_scope('Flatten'):
    with tf.name_scope('h_pool6_flat'):
        h_pool6_flat = tf.reshape(h_pool6, [-1, 12*4*128])
        print(h_pool6_flat)
...
with tf.name_scope('Fully_Connected_1'):
    with tf.name_scope('W_fc1'):
        W_fc1 = weight_variable([12*4*128, 512])
...
for epoch in range(1000):
    ...
    train_acc, train_loss = sess.run([Global_Average_Accuracy, loss], feed_dict={x: train_data[0:100], y: train_labels[0:100], keep_prob: 1.0})

    # Accuracy on Test Set
    test_summary, test_acc, test_loss = sess.run([merged, Global_Average_Accuracy, loss], feed_dict={x: test_data[0:100], y: test_labels[0:100], keep_prob: 1.0})
